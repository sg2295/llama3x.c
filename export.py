...
Only supports Huggingface weights.
...
def load_hf_model(model_path: str) -> Transformer:
...
def model_export(model: Transformer, filepath: str, version: int, dtype: torch.dtype=torch.float32):
    """
    Versions docs:
...
def torchscript_export(model: Transformer, filepath: str, zero_params: bool=False, gzip_output: bool=False):
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument("input_hf", type=str, help="huggingface model path")
    parser.add_argument("output_bin", type=str, help="the output filepath")
    parser.add_argument("--version", default=0, type=int, help="the version to export with")
    parser.add_argument("--dtype", type=str, help="dtype of the model (fp16, fp32)", default="fp32")
    args = parser.parse_args()
    dtype = {"fp16": torch.float16, "fp32": torch.float32}[args.dtype]

    model = load_hf_model(args.input_hf)

    if model is None:
        parser.error("Can't load input model!")

    if os.path.exists(args.output_bin):
        parser.error(f"Output file {args.output_bin} already exists! Remove file & retry")

    # export
    model_export(model, args.output_bin, args.version, args.dtype)
